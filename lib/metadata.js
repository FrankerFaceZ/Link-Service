...
export default class Metadata extends Resolver {
	...
	extractText(data, mode, ctx) {
		...
		const file_type = MIME_TYPES[ctx.response.mime];
	...
	extractMetadata(body, ctx) {
		...
		if ( og_data ) {
			...
			if ( og_data.image ) {
				...
				for (const image of og_data.image) {
					...
					try {
						url = this.service.normalizeURL(url, ctx.url);
					} catch (err) {
						continue;
					}
		...
		if ( ! images || ! images.length ) {
			...
			if ( raw_image )
				try {
					images = [{url: this.service.normalizeURL(raw_image, ctx.url)}];
				} catch (err) { /* no-op */ }
		}
		...
		if ( images && images.length > 0 ) {
			...
			if ( images.length > 1 )
				...
				const proxied = this.proxyImage(this.service.normalizeURL(images[0].secure_url || images[0].url, ctx.url));
				if ( proxied ) {
					if ( ! preview )
						preview = proxied;
				}
