...
import {InvalidHostError} from './errors/url';
...
class LinkService {
	...
	async resolve(url) {
		...
		while (url && redirects < this.opts.max_redirects) {
			const resolver = force_metadata ? this.metadata_resolver : this.pickResolver(url);
			if ( ! resolver ) {
...
LinkService.DEFAULT_OPTS = {
	...
	user_agent: 'Mozilla/5.0 (compatible; FFZBot/1.0; +https://www.frankerfacez.com)',
