...
function tokenizeTweet(text, entities, [start, end]) {
	...
	for (const {type, entity} of entities) {
		...
		if ( e_start > idx && e_start > start ) {
			tokens.push(ENTITIES.decode(text.slice(Math.max(start, idx), e_start).join('')));
			idx = e_start;
		}
		...
		if ( type === 'user_mentions' ) {
			...
			if ( type === 'urls' )
				...
				tokens.push(ENTITIES.decode(text.slice(e_start, e_end).join('')));
	...
	if ( idx < end )
		tokens.push(ENTITIES.decode(text.slice(idx, end).join('')));
...
export default class Twitter extends Resolver {
	...
	processBody(data, mode, ctx) {
		...
		return {
			...
			full: this.builder()
				.add(this.renderUserHeader(data, true, this.builder()
	...
	processTweet(data) {
		...
		if ( data.quoted_status ) {
			quoted = {
				type: 'link', embed: true, interactive: true, tooltip: false, href: quote_link, content: [
	...
	renderUserHeader(user, one_line = false, subtitle = null, extra = null) {
		let badges = [];
		if ( user.verified )
