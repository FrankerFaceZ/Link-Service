...
import DocumentBuilder, { boxToken, conditionalToken, formatToken, galleryToken, i18nToken, iconToken, imageToken, linkToken, overlayToken, refToken, styleToken } from '../builder';
...
const POST_URI_EXTRACTOR = /\/app.bsky.feed.post\/([^\/]+)/;
...
export default class Bluesky extends Resolver {
	...
	async transformURL(url, ctx) {
		...
		if ( url.hostname === 'bsky-feed.special.frankerfacez.com' ) {
			match = FEED_URL.exec(url.pathname);
			if ( match ) {
				ctx.skip_request = true;
				ctx.mode = 'user-feed';
				ctx.user_handle = match[1];
				ctx.cache_key = `bsky-feed-${match[1]}`;
		...
		match = POST_URL.exec(url.pathname);
		if ( match ) {
			ctx.skip_request = true;
			ctx.mode = 'post';
			ctx.user_handle = match[1];
	...
	async getFeed(user) {
		...
		let resp;
		try {
			resp = await this.agent.getAuthorFeed({
				actor: user,
				limit: 20,
		...
		if ( ! resp?.success || ! Array.isArray(resp.data?.feed) )
			return null;

		console.log('resp', resp);

		return {
			posts: resp.data.feed.map(x => x.post),
			has_more: !! resp.data.cursor?.length
		};
	}
	...
	async processBody(_, __, ctx, ___) {
		...
		if ( ctx.mode === 'user-feed' ) {
			const user = await this.getUser(ctx.user_handle),
				feed = await this.getFeed(user);

			return this.processFeed(user, feed, ctx);
		}

		return UseMetadata;
	}

	async processFeed(user, feed, ctx) {
		if ( ! feed?.posts?.length ) {
			console.log('what', feed);
			throw new NotFoundError;
		}

		const out = await Promise.all(feed.posts.map(post =>
			this.processPost(user, post, ctx, true)
		));
	...
	async processPost(user, post, ctx, in_feed = false) {
		...
		let url = undefined;
		if ( in_feed ) {
			let match = POST_URI_EXTRACTOR.exec(post.uri);
			if ( match )
				url = `https://bsky.app/profile/${user.handle}/post/${match[1]}`;
		}
		...
		if ( embed_record?.cid ) {
			...
			const match = POST_URI_EXTRACTOR.exec(embed_record.uri);
			if ( match )
		...
		let full = this.builder()
			.add(this.renderUserHeader(post.author, in_feed))
		...
		if ( in_feed ) {
			full = full
				.setFooter(
					null,
					url
						? linkToken(url, refToken('info'), {no_color: true, tooltip: false})
						: refToken('info')
				);
	...
	async renderBody(data, fragment, fragments) {
		...
		if ( data.author?.did && data.value?.embed ) {
			findGIFs(gifs, data.author.did, data.value.embed.images);
			findGIFs(gifs, data.author.did, data.value.embed.media?.images);
		}
...
Bluesky.hosts = [
	'bsky.app',
	'bsky-feed.special.frankerfacez.com'
];
