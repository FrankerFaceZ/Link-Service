...
import { parseHTML, tokenizeNode } from '../utilities';
...
const DummyCache = {
	...
	get(key, options) {
		...
		return Promise.resolve({hit: true, value: result.value});
	},
...
export default class Mastodon extends Resolver {
	...
	async fetchActivityUser(url) {
		...
		let cached = await cache.get(ckey);
		if ( cached.hit )
			return cached.value;
	...
	async isFediverse(host) {
		...
		const cache = this.service.opts.fedicache ?? this.service.opts.cache ?? DummyCache,
			ckey = `fed-check-${host}`;

		let resp = await cache.get(ckey);
		if (resp.hit)
			return resp.value;
		...
		await cache.set(ckey, resp.software.name);
