...
function tokenizeNode(node) {
	...
	if (tag === 'a') {
		// Link
		const ret = tokenizeNode(node.children);
		if (!ret)
			return ret;
...
function replaceEmoji(token, emoji, fragments) {
	...
	if (Array.isArray(token)) {
		...
		for(const tok of token) {
			const ret = replaceEmoji(tok, emoji, fragments);
			if (Array.isArray(ret)) {
				for(const r of ret)
					out.push(r);
			} else if (ret)
...
export default class Mastodon extends Resolver {
	...
	standardizeMastodonEmoji(emoji) {
		if (!Array.isArray(emoji) || emoji.length < 1)
			return null;

		const obj = {};
		for(const emo of emoji) {
			obj[emo.shortcode] = this.proxyImage(emo.url, '32,fit');
		}
		return obj;
	}

	standardizeActivityEmoji(tags) {
		...
		for(const tag of tags) {
			if (tag?.type === 'Emoji' && tag.icon?.type === 'Image' && tag.icon.url && tag.name) {
				obj[tag.name.slice(1, -1)] = this.proxyImage(tag.icon.url, '32,fit');
			}
		}
		return obj;
	}
	...
	async fetchActivityUser(url) {
		...
		if (raw && (raw.type === 'Person' || raw.type === 'Service'))
			result = {
				...
				emojis: this.standardizeActivityEmoji(raw.tag)
			};
	...
	async isFediverse(host) {
		...
		// Get the node info.
		resp = await fetchJSON(`https://${host}/.well-known/nodeinfo`);
	...
	async processMastodonUser(data, ctx) {
		...
		if ( Array.isArray(data.emojis) && data.emojis.length > 0 ) {
			data.emojis = this.standardizeMastodonEmoji(data.emojis);
			has_emoji = true;
		...
		if ( data.header && ! data.header.includes('missing') )
			header = conditionalToken(true, true, galleryToken(
				imageToken(this.proxyImage(data.header, '320x180,fit', {aspect:16/9}))
			));
		...
		return {
			...
			full: this.builder()
				.add(this.renderUserHeader(
					...
					null,
					imageToken(
						this.proxyImage(data.header, '320x180,fit'),
						{
							aspect: 16/9
						}
					)
				...
				.add(this.renderBody(data, false))
				.add(field_table)
				//.add(header)
	...
	async processActivityUser(data, ctx) {
		...
		const fragments = {},
			...
			emoji = this.standardizeActivityEmoji(data.tag),
			site = await this.fetchSiteStyle(ctx.url.host),
		...
		const image = data.image?.type === 'Image' && data.image.url
			? conditionalToken(true, true, galleryToken(
				imageToken(this.proxyImage(data.image.url))
			))
			: null;
		...
		return {
			...
			short: this.renderUserHeader(
				...
				site?.icon
					? [
						imageToken(this.proxyImage(site.icon, '32,fit'), {height: '1.3em'}),
						' ',
						refToken('footer')
					]
					: refToken('footer')
			...
			full: this.builder()
				...
				.setFooter(
					null,
					refToken('footer'),
					site?.icon ? this.proxyImage(site.icon, '32,fit') : null
				)
		};
	}
	...
	async processActivityNote(data, ctx) {
		...
		if (Array.isArray(data.tag) && data.tag.length > 0)
			body = replaceEmoji(body, this.standardizeActivityEmoji(data.tag), fragments);
		...
		return {
			...
			full: this.builder()
				.add(this.renderUserHeader(user, false, fragments))
				.add(this.renderBody(data, false))
				.setFooter(
					null,
					refToken('footer'),
	...
	processToot(data) {
		...
		let body = tokenizeNode(parseHTML(data.content));
		if (Array.isArray(data.emojis) && data.emojis.length > 0)
			body = replaceEmoji(body, this.standardizeMastodonEmoji(data.emojis), fragments);

		data.account.emojis = this.standardizeMastodonEmoji(data.account.emojis);
		...
		return {
			...
			full: this.builder()
				.add(this.renderUserHeader(data.account, false, fragments))
				.add(this.renderBody(data, warning ? true : false))
	...
	renderBody(data, has_warning = false) {
		const raw_media = data.media_attachments;
		let media;
		if (raw_media?.length > 0) {
			if (raw_media.length > 4)
				raw_media = raw_media.slice(0, 4);

			media = raw_media.map(entity => {
				let token;
				if (entity.type === 'image')
					token = imageToken(
						this.proxyImage(entity.preview_url),
						{
					...
					token = overlayToken({
						...
						attrs: {
							...
							loop: true,
							autoplay: true,
							poster: this.proxyImage(entity.preview_url),
							title: entity.description
						},
						content: [
							{
								...
								tag: 'source',
								attrs: {
									src: this.proxyImage(entity.url),
									type: 'video/mp4'
								}
							}
						]
					...
					{
						'bottom-left': i18nToken('embed.mastodon.gif', 'GIF')
					});
					/*token = imageToken(
						this.proxyImage(entity.preview_url),
						{
				...
				return linkToken(entity.url, token, {tooltip: false});
			}).filter(x => x);
			...
			if ( data.card.type === 'link' ) {
				content = {
					...
					image: data.card.image
						? imageToken(this.proxyImage(data.card.image), {aspect: 1})
						: iconToken('docs')
				};
				...
				if ( data.card.image ) {
					if ( data.card.type === 'video' )
						content = content
							.addConditional(true, true, overlayToken(
								imageToken(this.proxyImage(data.card.image), {aspect: data.card.width / data.card.height}),
								{
									'center': styleToken({size: '1'}, iconToken('play'))
								}
							));
	...
	renderUserHeader(user, one_line = false, fragments = null, subtitle = null, extra = null, background = null) {
		let badges = [];
		if (user.bot)
			badges.push({
				type: 'tag',
		...
		let builder = this.builder()
			.setLogo(this.proxyImage(user.avatar));
...
Mastodon.examples = [
	...
	{title: 'Post with Media', url: 'https://mastodon.social/@SirStendec/109321982274432188'},
