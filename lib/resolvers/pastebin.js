...
export default class Pastebin extends Resolver {
	...
	async processBody(body, mode, ctx) {
		if ( ! body || mode !== 'html' )
			return;
		...
		const lines = body('.post-view > .highlighted-code .source li');
		let raw = [];

		for(let i = 0; i < lines.length; i++)
			raw.push(body(lines[i]).text().trimEnd());
		...
		const avatar_or_logo = await this.proxyImage(avatar ?? LOGO);
		...
		return {
			...
			short: this.builder()
				.setLogo(await this.proxyImage(LOGO), {sfw: true, aspect: 1})
			...
			mid: this.builder()
				.setCompactHeader()
				.setLogo(avatar_or_logo, {sfw: avatar ? false : true, aspect: 1})
				.setTitle(refToken('title'))
				.setSubtitle(refToken('info'))
				.addBox({
					wrap: 'pre-wrap',
