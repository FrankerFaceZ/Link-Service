...
export default class OpenCourseWorld extends Resolver {
	...
	async processMaker(data, ctx) {
		if ( ! data?.id )
			return UseMetadata;

		const fragments = {
			logo: imageToken(await this.proxyImage(LOGO)),
			avatar: imageToken(await this.proxyImage(data.image_url), {sfw: false}),
			title: [
				data.username,
				' ',
				styleToken({
	...
	async processCourse(data, ctx) {
		...
		const fragments = {
			...
			thumb: imageToken(await this.proxyImage(thumbnail)),
			user: linkToken(
		...
		// TODO: Tags? When tags in API at least
