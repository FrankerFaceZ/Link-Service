...
export default class SevenTV extends Resolver {
	...
	processBody(data) {
		...
		const flags = data.visibility,
			...
			Unlisted				= flags >>> 2 & 1,
			ZeroWidth				= flags >>> 7 & 1,
			PermanentlyUnlisted		= flags >>> 8 & 1,
			approved = !(Unlisted || PermanentlyUnlisted),
		...
		const user = linkToken(
			...
			emote = Global ? i18nToken('embed.7tv.global_emote', 'Global Emote') : i18nToken('embed.7tv.emote', 'Emote'),
		...
		if (Unlisted || PermanentlyUnlisted) {
			visibilityFlags.push(' • ');
		...
		return {
			...
			short: this.builder()
				.setLogo(image, {sfw: approved, aspect: aspect})
			...
			full: this.builder()
				...
				.addConditional(true, approved ? undefined : true,
					galleryToken([
						overlayToken(
							preview_image,
							{},
