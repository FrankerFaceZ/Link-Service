...
import RuntimeError, {UnhandledURLError, TimeoutError, NetworkError, NotFoundError, BadRequestError, UnauthorizedError, ForbiddenError, ServerError, RedirectLoopError} from './errors/runtime';
import DocumentBuilder from './builder';
import { removeTracking } from './normalize-url';
import { finishObject } from './utilities';

//const REFRESH_TARGET = /\d+;\s*url=(.+)$/i;
const REFRESH_TARGET = /^\s*(\d+)(?:\s*;(?:\s*url\s*=)?\s*(?:["']\s*(.*?)\s*['"]|(.*?)))?\s*$/i;
...
class Resolver {
	...
	/**
	 * When making an HTTP request, limit the request to this many bytes of
	 * content. This sets a `Range` header on outgoing requests, as well as
	 * preventing any body reading operations from reading more than that
	 * many bytes. Return zero, or a negative value, for no limit.
	 *
	 * @param {URL} url The URL we're processing.
	 * @param {RequestContext} ctx A context object that will be maintained
	 * while processing this URL to keep track of extra data.
	 * @returns The maximum number of bytes to parse.
	 */
	limitResponseSize(url, ctx) { // eslint-disable-line no-unused-vars
		return 1_000_000;
	}


	async _run(url, referrer, cookies, hsts) {
		const ctx = {
			url,
			original_url: url,
			referrer,
		...
		if ( ! request_url && ! (ctx.skip_request && ctx.cache_key) )
		...
		if ( this.service.cache && ctx.cache_key !== false ) {
			if ( ctx.cache_key == null ) {
				// Remove a bit of entropy for random URLs by sorting query parameters
				// as well as removing tracking parameters.
				const cache_url = removeTracking(new URL(request_url));
				cache_url.searchParams.sort();
				ctx.cache_key = cache_url.toString();
			}
		...
		if ( ! ctx.skip_request && ! ctx.fetch && !(request_url instanceof URL) )
		...
		if ( ! ctx.skip_request ) {
			...
			limit = await this.limitResponseSize(url, ctx);
			...
			if ( this.service?.opts?.http_proxy )
				options.http_proxy = this.service.opts.http_proxy;
			...
			while(true) {
				// First things first, handle HSTS.
				if ( hsts )
					request_url = hsts.upgrade(request_url);
				...
				try {
					request = ctx.request = await req_fetch(request_url, options, ctx.cookies, ctx.hsts);
				...
				status = ctx.status = request.status;
				...
				if ( ctx.follow_redirects ) {
					...
					if ( redirect ) {
						...
						let target = new URL(redirect, request_url);
						if ( hsts )
							target = hsts.upgrade(target);

						const target_str = target.toString();

						/*console.log('current', req_str);
						console.log(' target', target_str);
						console.log('referer', ctx.referrer?.toString?.());
						console.log('->', Object.fromEntries(request.headers.entries()));//*/

						// TODO: Decide if setting a cookie should let this one pass.
						if ( target_str === req_str && target_str === ctx.referrer?.toString?.() )
							throw new RedirectLoopError();

						// In immediate mode, keep going.
						if ( immediate ) {
							ctx.referrer = request_url;
							request_url = target;
							continue;

						} else {
							// Otherwise, return a redirect response.
							const out = new Redirect(target, ctx.url);
			...
			if ( wants_body === UseMetadata || wants_body instanceof UseMetadata || wants_body instanceof Redirect ) {
				...
				try {
					if ( wants_body === 'buffer' ) {
						...
						body = await request.jsonLimit(limit);
						mode = 'json';

					} else if ( wants_body === 'html' ) {
						const raw = await request.textLimit(limit);
						...
						body = await request.textLimit(limit);
						mode = 'text';
					}
			...
			if ( ! data ) {
				let log = this.service.opts.log_error_responses ?? false; // true;
				if ( status === 404 )
				...
				if ( log ) {
					try {
						const body = await request.textLimit(limit);
						console.log('GET', String(ctx.url), status, '=>', body.length, body);
					} catch(err) { /* no-op */ }
				}
			}
