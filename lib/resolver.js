...
import {UnhandledURLError, TimeoutError} from './errors/runtime';
 ...
 * the same resource.
 * @property {Object} [cache_opts] An optional, opaque object of extra
 * options to be passed to cache methods. This can be used to override
 * default timeouts, etc. depending on your {@link CacheProvider}.
...
class Resolver {
	...
	async _run(url, referrer) {
		...
		if ( this.cache ) {
			...
			if ( ctx.cache_key ) {
				const resp = await this.cache.get(ctx.cache_key, ctx.cache_opts);
				if ( resp?.hit ) {
		...
		if ( ctx.follow_redirects ) {
			...
			if ( redirect ) {
				...
				if ( this.cache && ctx.cache_key )
					await this.cache.set(ctx.cache_key, out, ctx.cache_opts);
